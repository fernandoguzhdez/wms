...
export const SeriesLotesCompras = ({route}) => {
  ...
  const [filteredWarehouses, setFilteredWarehouses] = useState([]); // listado filtrado
  ...
  const obtenerAlmacen = () => {
    ...
    axios
      .get(`${url}/api/Inventory/Get_WhareHouse`, {headers})
      .then(response => {
        ...
        setFilteredWarehouses(whsData);

        // 👇 buscamos en el listado el almacén que coincide con detalle.WhsCode
        const preselected = whsData.find(
            (wh) => wh.key.toUpperCase() === detalle.WhsCode.toUpperCase()
          );
  
          if (preselected) {
            setSelectedWarehouse(preselected.key); // lo seleccionamos automáticamente
            console.log("Almacen seleccionado...", preselected.key)
          }
      })
      .catch(error => {
        console.error(error);
  ...
  const handleSearchAlmacen = (text) => {
    if (text) {
      ...
      // 👈 si está vacío, regresa al listado completo
  ...
  const eliminarRegistro = async item => {
    Alert.alert(
      ...
      `¿Seguro que quieres eliminar ${
        esSerie ? 'la serie' : esLote ? 'el lote' : 'el registro'
      } ${item.idCode}?`,
      [
            ...
            try {
              ...
              if (response.status === 200) {
                ...
                Alert.alert(
                  ...
                  `${
                    esSerie ? 'La serie' : esLote ? 'El lote' : 'El registro'
                  } ${item.idCode} fue eliminado correctamente.`,
                );
              } else {
  ...
  const handleGuardar = async () => {
    if (!nombreSerieLote.trim()) {
      Alert.alert(
        'Campo requerido',
        'Por favor ingresa un nombre de serie o lote.',
      );
      return;
    }
    ...
    const sumaCantidadExistente = listaSeriesLotes.reduce(
      (acc, curr) => acc + (curr.quantityCounted || 0),
      0,
    );
    ...
    if (esSerie) {
      const existeSerie = listaSeriesLotes.find(
        x => x.idCode === nombreSerieLote,
      );
      if (existeSerie) {
        Alert.alert(
          'Serie duplicada',
          'Esta serie ya fue ingresada previamente.',
        );
        return;
      }
    }
    ...
    let serManLineNum = 0;
    let existente = listaSeriesLotes.find(
      x =>
    ...
    const body = {
      ...
      binCode: binCode,
      whsCode: Number(selectedWarehouse),
    ...
    try {
      ...
      Alert.alert(
        'Éxito',
        `${esSerie ? 'Serie' : 'Lote'} guardado correctamente.`,
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        placeholder={`Buscar ${
          esSerie ? 'serie' : esLote ? 'lote' : 'serie o lote'
        }...`}
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalBackground}>
          <View style={styles.modalContainer}>
            ...
            <SelectList
              setSelected={val => setSelectedWarehouse(val)}
              data={warehouses}
              save="key" // aquí guardas el código del almacén (whsCode)
              placeholder="Selecciona un almacén"
              searchPlaceholder="Buscar almacén..."
              search={true} // 🔍 activa búsqueda local
              onTextChange={handleSearchAlmacen}
              defaultOption={
                selectedWarehouse
                  ? {
                      key: selectedWarehouse,
                      value:
                        warehouses.find((wh) => wh.key === selectedWarehouse)?.value ||
                        "",
                    }
                  : null
              } // 👈 aquí mostramos el valor preseleccionado
